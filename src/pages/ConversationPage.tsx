...
const ConversationPage = () => {
  ...
  // Tampilan utama
  return (
    <div className="absolute top-0 left-0 z-10 h-full w-full bg-white transition-transform duration-300 ease-in-out">
      <div className="flex h-full flex-col">
        <ChatHeader
          participantCount={liveComments.length + 1}
          title={post?.subject || 'Conversation'}
        />

        <div
          className="flex-grow space-y-4 overflow-y-auto bg-white p-4"
        ...
        >
          {liveComments.map((comment) => {
            ...
            if (comment.replyTo) {
              const original = liveComments.find(
            ...
            return (
              <Fragment key={comment.id}>
                {comment.id === firstUnreadId && (
                  <NewMessageSeparator isAtBottom={isAtBottom} />
                )}
                <MessageBubble
                  comment={comment}
        ...
        {replyTo && (
          <div className="flex items-center rounded-t-lg border-gray-700 border-b bg-gray-800 px-4 py-2 text-white">
            <div className="flex-1">
              <span className="text-gray-400 text-xs">
                Replying to {replyTo.owner.firstName} {replyTo.owner.lastName}
              </span>
              <div className="truncate text-gray-200 text-sm">
                {replyTo.message}
              </div>
            </div>
